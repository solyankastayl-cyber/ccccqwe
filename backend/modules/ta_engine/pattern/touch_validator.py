...
class TouchValidator:
    ...
    def calculate_touch_score(
        ...
        Factors:
        - Number of touches per side
        - Reaction quality
        - Touch precision
        """
        upper_count = len([t for t in upper_touches if t.is_reaction])
        lower_count = len([t for t in lower_touches if t.is_reaction])
        
        # Minimum requirement
        if upper_count < min_touches_per_side or lower_count < min_touches_per_side:
            return 0.0
        
        # Base score from touch counts
        total_touches = upper_count + lower_count
        count_score = min(1.0, total_touches / 6.0)  # Max score at 6 touches
        ...
        # Combined score
        score = (count_score * 0.4 + reaction_ratio * 0.4 + avg_precision * 0.2)
